...
#include "File/FileSystemController_LMBR.h"
...
void	PopcornFXIntegration::Deactivate()
{
	...
	if (scene)
	{
		...
		if (pkfxFeatureProc)
		{
			pkfxFeatureProc->Init(nullptr, nullptr);
...
void	PopcornFXIntegration::ReloadAssets()
{
	...
	AZ::Data::AssetCatalogRequestBus::Broadcast(&AZ::Data::AssetCatalogRequestBus::Events::EnumerateAssets, nullptr, popcornFxAssetReloadCb, nullptr);
}

void	PopcornFXIntegration::PreloadMesh(const AZStd::string &meshPath)
{
	m_Preloader.PreloadMesh(meshPath);
}
...
void	PopcornFXIntegration::SpawnEmitterAtLocation(const AZStd::string& path, AZ::Transform transform)
{
	SpawnEffectByName(path, PopcornFX::SpawnParams(true, true, transform));
}

AZ::EntityId	PopcornFXIntegration::SpawnEmitterEntityAtLocation(const AZStd::string& path, AZ::Transform transform, bool start)
{
