...
#include "Integration/File/FileSystemController_LMBR.h"
...
#define		USE_LUMBERYARD_JOBS		0

#if		USE_LUMBERYARD_JOBS
	struct	SLmbrJob : public AZ::Job
	{
		AZ_CLASS_ALLOCATOR(SLmbrJob, AZ::ThreadPoolAllocator, 0);

		SLmbrJob(AZ::JobContext* context = nullptr)
			: AZ::Job(true, context)
			, m_PkfxTask(null)
	...
	class	CLmbrThreadPool : public Threads::CAbstractPool
	{
	public:
		CLmbrThreadPool()
			: m_Context(this, 1)
		...
		virtual	CStringView	ThreadPoolName() const override { return "Lumberyard PopcornFX thread pool"; };

	private:
		Threads::SThreadContext				m_Context;
	};
	PK_DECLARE_REFPTRCLASS(LmbrThreadPool);
#endif
...
#if		USE_LUMBERYARD_JOBS
		// Lumberyard thread pool:
		CLmbrThreadPool				*threadPool = PK_NEW(CLmbrThreadPool);
		return threadPool;
#else
	...
	{
		return PK_NEW(FileSystemController_LMBR);
	}
...
#	endif
			...
			Kernel::CheckStaticConfigFlags(Kernel::g_BaseStaticConfig, SKernelConfigFlags()))
		{
			...
			AdditionalRendererProperties::Startup();
	...
	void	PopcornShutdown()
	{
		...
		AdditionalRendererProperties::Shutdown();
