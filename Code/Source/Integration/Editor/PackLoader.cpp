...
AZStd::string	FindFileInHierarchy(const AZStd::string &folder, const AZStd::string &file)
{
	...
	while (filePath.empty())
	{
		...
		AZ::StringFunc::Path::Join(folderPath.c_str(), file.c_str(), tryProjPath);
		...
		{
			if (!AZ::StringFunc::Path::StripComponent(folderPath, true) ||
	...
	AZ::StringFunc::Path::Normalize(filePath);
	return filePath;
}
...
AZStd::string	FindPkprojWithRootDirOnDev(const AZStd::string &devFolderPath)
{
	...
	for (size_t i = 0; i < pkprojFiles.size(); ++i)
	{
		...
		{
			AZ::StringFunc::Path::Normalize(pkprojFiles[i]);
...
void	GetThumbnailPathForAsset(const AZStd::string &assetPath, AZStd::string &outThumbnailPath)
{
	...
	if (pkProjPath.empty())
	{
		...
		if (devFolderPath.empty())
		{
			...
			AZ::StringFunc::Path::StripFullName(devFolderPath);
			AZ::StringFunc::Path::Normalize(devFolderPath);
			AZ::StringFunc::Strip(devFolderPath, AZ_CORRECT_FILESYSTEM_SEPARATOR, false, false, true);
		}
		if (!devFolderPath.empty())
								...
								const AZStd::string &devFolder /*= ""*/)
{
	...
	if (packs.Count() > 0)
	{
		...
		if (!devFolder.empty())
			AZ::StringFunc::Path::Join(devFolder.c_str(), packPath.c_str(), packPath);
