...
PopcornFXRendererLoader::~PopcornFXRendererLoader()
{
	...
	AZ::RPI::ShaderVariantFinderNotificationBus::MultiHandler::BusDisconnect();
...
void	PopcornFXRendererLoader::OnShaderVariantTreeAssetReady(AZ::Data::Asset<AZ::RPI::ShaderVariantTreeAsset> shaderVariantTreeAsset, bool isError)
{
	(void)isError;
	if (isError)
		return;
	const AZ::Data::AssetId	*shaderId = AZ::RPI::ShaderVariantFinderNotificationBus::GetCurrentBusId();
	
	if (!PK_VERIFY(shaderId != null))
		return;
    CLog::Log(PK_INFO, "_OnShaderVariantsReloaded (OnShaderVariantTreeAssetReady)");
	_OnShaderVariantsReloaded(*shaderId, AZ::RPI::ShaderVariantId(), false);
    CLog::Log(PK_INFO, "(EXIT OnShaderVariantTreeAssetReady)");
}
...
void	_SetMeshInfos(PLmbrRendererCache rendererCache, const PGeometryCache geometryCache)
{
...
void	PopcornFXRendererLoader::_OnAssetReady(AZ::Data::Asset<AZ::Data::AssetData> asset)
{
	...
	if (asset.GetType() == azrtti_typeid<AZ::RPI::ShaderAsset>())
	{
		...
		for (const SAssetDependencies::SCaches &cache : dependencies->m_Caches)
		{
			...
			{
				AZ::RPI::ShaderVariantFinderNotificationBus::MultiHandler::BusConnect(shaderAsset.GetId());
				AZ::RPI::ShaderReloadNotificationBus::MultiHandler::BusConnect(shaderAsset.GetId());

				//const AZ::RPI::ShaderOptionGroupLayout	*options = shaderAsset->GetShaderOptionGroupLayout();
				...
				if (cache.m_Type == AssetType_MaterialShader)
				{
					AZ::RPI::ShaderVariantId            variantId = cache.m_PipelineStateKey.GetShaderVariantId(*shader, true);
				...
				pipelineStateSlot = _CreatePipelineStateCache(*shader, cache.m_Type, variantStableId, cache.m_PipelineStateKey);
	...
	else if (asset.GetType() == azrtti_typeid<AZ::RPI::ModelAsset>())
	{
		...
		// Cache found: setup with model data
		{
			...
			TArray<PLmbrRendererCache> *rendererCaches = m_RendererCacheDependencies.Find(asset.GetId());
			if (rendererCaches != null && geometryCache != null)
			{
				for (PLmbrRendererCache rendererCache : *rendererCaches)
				{
...
void	PopcornFXRendererLoader::_OnShaderVariantsReloaded(const AZ::Data::AssetId &shaderId, const AZ::RPI::ShaderVariantId &shaderVariantId, bool searchVariant)
{
	PK_SCOPEDLOCK(m_Lock);
	...
	for (const SAssetDependencies::SCaches &cache : dependencies->m_Caches)
	{
		...
		if (pipelineStateCache != null && *pipelineStateCache != null)
		{
			...
			if (!searchVariant || isSameVariantId)
			{
				PK_SCOPEDLOCK_WRITE((*pipelineStateCache)->m_Lock);
                AZ::RHI::ConstPtr<AZ::RHI::PipelineState>   &pipelineStateSlot = _GetPipelineStateSlot(cache.m_Type, *pipelineStateCache);
				// Triggers variant asset tree load:
   				pipelineStateSlot = _CreatePipelineStateCache(*shader, cache.m_Type, variantStableId, cache.m_PipelineStateKey);
   				(*pipelineStateCache)->m_Modified = true;
                variantFound = true;
			}
...
{
	AZ::RPI::ShaderVariant					shaderVariant = shader.GetVariant(shaderVariantStableId);
    ...
    if (assetType == AssetType_MaterialShader)
    {
    	...
    	if (IsLegacyShader(usedShader))
    	{
    		...
    		{
    			PK_ASSERT_NOT_REACHED_MESSAGE("Blend mode is not handled...");
    		}
    	}
    }
...
bool PopcornFXRendererLoader::_InsertRendererCacheDependency(const AZ::Data::AssetId& assetId, const PLmbrRendererCache &rendererCache)
{
	if (!assetId.IsValid())
		return false;

	TArray<PLmbrRendererCache> *dependencies = m_RendererCacheDependencies.Find(assetId);
	if (dependencies == null)
	{
		dependencies = m_RendererCacheDependencies.Insert(assetId, TArray<PLmbrRendererCache>());
		if (!PK_VERIFY(dependencies != null))
			return false;
...
bool		PopcornFXRendererLoader::AddGeometryToLoad(const CString &path, const PLmbrRendererCache &rendererCache)
{
	PK_SCOPEDLOCK(m_Lock);
