...
#define DRAW_POOL(__name, __pool)													\
    ...
    */
	...
	for (u32 i = 0; i < memNodeCount; ++i)
	{
		...
		DrawVBar(offperc, offmem - 1, 0.0f, perc, 100.f, lineHeight);
		{
...
void	CStatsManager::DrawProfiler()
{
	...
	for (u32 iTiming = 0; iTiming < allSceneTimings.Count(); ++iTiming)
	{
		...
		DrawBar(relSimTimexPos, relRenderTimexPos - 3, 0.0f, effectSimTimeRelative / 100.0f, lineHeight);
		DrawText(relSimTimexPos, 0.0f, AZStd::string::format("%.1f", effectSimTimeRelative));
		DrawBar(relRenderTimexPos, effectSimTimexPos - 3, 0.0f, effectRenderTimeRelative / 100.0f, lineHeight);
	...
	DrawBar(relSimTimexPos, relRenderTimexPos - 3, 0.0f, displayedSimEffectsPercentage, lineHeight);
	DrawText(relSimTimexPos, 0.0f, AZStd::string::format("%.1f", displayedSimEffectsPercentage * 100.0f));
	DrawBar(relRenderTimexPos, effectSimTimexPos - 3, 0.0f, displayedRenderEffectsPercentage, lineHeight);
...
void	CStatsManager::DrawBar(float minX, float maxX, float yPos, float cursor, float thickness) const
{
	(void)minX;
	(void)maxX;
	(void)yPos;
	...
	Draw2dHelper helper;
	helper.DrawImage(m_whiteTex, AZ::Vector2(minX, yPos), AZ::Vector2(sizeX, thickness), 0.6f);
	helper.DrawImage(m_whiteTex, AZ::Vector2(minX + sizeX, yPos), AZ::Vector2((maxX - minX) - sizeX, thickness), 0.1f);
#endif
}

void	CStatsManager::DrawVBar(float minX, float maxX, float y, float value, float maxValue, float thickness) const
{
	(void)minX;
	(void)maxX;
	(void)y;
	...
	Draw2dHelper helper;
	AZ::Vector3 color(c, 1.f - c, 0.0f);
	helper.DrawImage(m_whiteTex, AZ::Vector2(minX, y), AZ::Vector2(sizeX, thickness), 0.6f);
	helper.DrawImage(m_whiteTex, AZ::Vector2(minX + sizeX, y), AZ::Vector2((maxX - minX) - sizeX, thickness), 0.1f);
#endif
}
